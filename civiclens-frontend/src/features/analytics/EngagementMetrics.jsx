    ...
    <div className="glass-card p-4 sm:p-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        {metrics.map((m) => (
          <div
            key={m.label}
            className="bg-dark-800/50 rounded-xl p-2.5 sm:p-3 border border-dark-700/30
          ...
          >
            ...
            <span className={`text-lg sm:text-xl font-bold ${m.color}`}>
      ...
      {analytics.totalVotes > 0 && (
        <div className="mt-3 sm:mt-4">
          <div className="flex justify-between text-[10px] sm:text-xs text-dark-500 mb-1 sm:mb-1.5">
            <span>Vote Polarity</span>
            <span>{((analytics.votePolarity || 0) * 100).toFixed(0)}%</span>
          </div>
          <div className="h-1.5 sm:h-2 bg-dark-800 rounded-full overflow-hidden flex">
            <div
