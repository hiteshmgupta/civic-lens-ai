    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        <h1 className="text-2xl font-bold text-dark-100 flex items-center gap-3">
          <div className="w-10 h-10 bg-civic-400/20 rounded-xl flex items-center justify-center">
      ...
      {/* Tabs */}
      <div className="flex items-center bg-dark-900/60 rounded-xl p-1 border border-dark-700/50 w-fit">
        {tabs.map((tab) => (
          <button
            key={tab.id}
...
function StatCard({ label, value, icon, color }) {
  return (
    <div className="stat-card hover:border-civic-400/20 transition-all duration-200">
      <div className="flex items-center gap-2">
        <span className="text-lg">{icon}</span>
        <span className="text-xs text-dark-500 uppercase tracking-wider">{label}</span>
      </div>
      <span className={`text-2xl font-bold ${color} mt-1`}>
